...
import (
	...
	"forensic-tools/internal/adapter/messaging"
	"forensic-tools/internal/adapter/repository"
	"forensic-tools/internal/adapter/storage"
	"forensic-tools/internal/config"
	"forensic-tools/internal/core/ports"
	"forensic-tools/internal/core/service"
	"forensic-tools/internal/handler"
)
...
func main() {
	...
	logger.Info("Starting Forensic Tools Service", "name", cfg.App.Name, "env", cfg.App.Environment)
	...
	if cfg.Storage.Type == "local" {
		storageBackend, err = storage.NewLocalStorage(cfg.Storage.BasePath, logger)
		if err != nil {
			log.Fatalf("Failed to initialize local storage: %v", err)
		}
		logger.Info("Local storage initialized", "path", cfg.Storage.BasePath)
	} else {
	...
	if cfg.Messaging.Type == "kafka" {
		...
		if kafkaProducer, ok := messagingClient.(*messaging.KafkaProducer); ok {
			if err := kafkaProducer.EnsureTopics(ctx, []string{
				"evidence.collected",
				"analysis.started",
				"analysis.completed",
				"custody.transfer",
	...
	// Initialize service
	forensicService := service.NewForensicService(repo, storageBackend, messagingClient, logger)

	// Initialize handler
	forensicHandler := handler.NewForensicHandler(forensicService, logger)
	...
	// Register routes
	forensicHandler.RegisterRoutes(router)

	// Health check endpoint
	router.GET("/health", func(c *gin.Context) {
		forensicHandler.HealthCheck(c)
	})
...
func authMiddleware(jwtSecret string, logger ports.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Set user_id from header for demo purposes
