...
import (
	...
	"control-layer/internal/adapter/messaging"
	"control-layer/internal/adapter/repository"
	"control-layer/internal/config"
	"control-layer/internal/core/ports"
	"control-layer/internal/core/service"
	"control-layer/internal/handler"
)
...
func main() {
	...
	logger.Info("Starting Control Layer Service", "name", cfg.App.Name, "env", cfg.App.Environment)
	...
	repo := repository.NewPostgresPolicyRepository(db, logger)

	// Initialize cache (using simple in-memory for now)
	cache := NewMemoryCache(logger)

	// Initialize Kafka producer
	var messagingClient ports.MessagingClient
	if cfg.Messaging.Type == "kafka" {
		messagingClient = messaging.NewKafkaProducer(cfg.Messaging.Brokers, cfg.Messaging.TopicPrefix, logger)
		logger.Info("Kafka messaging initialized", "brokers", cfg.Messaging.Brokers)
	}

	// Initialize service
	policyService := service.NewPolicyService(repo, cache, messagingClient, logger)
...
// MemoryCache implements a simple in-memory cache for access decisions
...
func NewMemoryCache(logger ports.Logger) *MemoryCache {
	return &MemoryCache{
...
func (c *MemoryCache) Get(ctx interface{}, key string) (interface{}, bool) {
...
func (c *MemoryCache) Set(ctx interface{}, key string, value interface{}, ttl int) error {
	c.cache[key] = &struct {
		response *struct {
			Allowed   bool
			Reason    string
...
func (c *MemoryCache) Delete(ctx interface{}, key string) error {
	delete(c.cache, key)
	return nil
}

func (c *MemoryCache) InvalidateByPattern(ctx interface{}, pattern string) error {
