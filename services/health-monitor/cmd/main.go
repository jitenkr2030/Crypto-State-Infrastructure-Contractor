...
import (
	...
	"health-monitor/internal/adapter/messaging"
	"health-monitor/internal/adapter/repository"
	"health-monitor/internal/config"
	"health-monitor/internal/core/ports"
	"health-monitor/internal/core/service"
	"health-monitor/internal/handler"
)
...
func main() {
	...
	logger.Info("Starting Health Monitor Service", "name", cfg.App.Name, "env", cfg.App.Environment)
	...
	repo := repository.NewPostgresHealthRepository(db, logger)
	...
	// Initialize service
	healthService := service.NewHealthMonitorService(repo, messagingClient, logger)

	// Initialize handler
	healthHandler := handler.NewHealthMonitorHandler(healthService, logger)
	...
	go func() {
		if err := healthService.StartActiveMonitoring(context.Background()); err != nil {
			logger.Error("Failed to start active monitoring", "error", err)
		}
	}()
	...
	healthService.StopActiveMonitoring(context.Background())
